...
**边界：用户直接提供并明确声明为最终正文的完整章节，按 Manifest Authority Order 第1级处理；Gate不得反向改写已发布用户正文，但必须把偏离记录为未来章节回归风险。**
...
允许展示完整 Assistant Candidate 的必要条件：
...
用户直接给出完整最终稿并明确发布时，按Manifest的USER_FINAL_OVERRIDE边界处理：正文直接成为发布权威，Gate只记录偏离，不得改字；下一章恢复正常流程。
